...
public class B148525512 extends KotlinTestBase {
  ...
  private static final KotlinCompileMemoizer kotlinBaseClasses =
      getCompileMemoizer(getKotlinSourceFileFromResources(pkg, "base"))
          .configure(
  ...
  private static final KotlinCompileMemoizer kotlinFeatureClasses =
      getCompileMemoizer(getKotlinSourceFileFromResources(pkg, "feature"))
          .configure(
              kotlinCompilerTool -> {
            ...
            .inspectWithSyntheticItems(
                (inspector, syntheticItems) -> {
                  if (kotlinParameters.getLambdaGeneration().isClass()) {
                    ...
                    assertRemovedFromOutput(
                        syntheticItems.syntheticLambdaClass(baseKt, 0), inputInspector, inspector);
                    assertRemovedFromOutput(
                        syntheticItems.syntheticLambdaClass(baseKt, 1), inputInspector, inspector);
                    assertRemovedFromOutput(
                        syntheticItems.syntheticLambdaClass(featureKt, 0),
                        inputInspector,
                        inspector);
                    assertRemovedFromOutput(
                        syntheticItems.syntheticLambdaClass(featureKt, 1),
                        inputInspector,
                        inspector);
                  }
                });
