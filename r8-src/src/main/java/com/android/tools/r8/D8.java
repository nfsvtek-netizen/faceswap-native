...
public final class D8 {
  ...
  static void runInternal(AndroidApp inputApp, InternalOptions options, ExecutorService executor)
    ...
    try {
      ...
      Marker marker =
          hasClassResources || options.enableDexToDexCodeOptimizations ? options.getMarker() : null;
  ...
  public static class ApkAnalyzerEntryPoint {

    public static void run(D8Command command, Consumer<InternalOptions> optionsModification)
        throws CompilationFailedException {
      AndroidApp app = command.getInputApp();
      InternalOptions options = command.getInternalOptions();
