  ...
  public List<VirtualFile> run(Timing timing) throws ExecutionException {
    ...
    if (multidexLegacy && options.enableInheritanceClassInDexDistributor) {
      new InheritanceClassInDexDistributor(
              ...
              executorService)
          .distribute();
    } else if (options.enablePreserveExistingClassToDexDistributor) {
      new PreserveExistingClassToDexDistributor(
              virtualFiles, filesForDistribution, appView, classes, nextFileId)
